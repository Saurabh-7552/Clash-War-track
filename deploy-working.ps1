# Working Deployment Script for Clash War Tracker
param(
...
ssh -i "$KeyPath" ubuntu@$ElasticIP "sudo apt update && sudo apt install -y default-jdk maven nodejs npm postgresql postgresql-contrib nginx unzip"

Write-Host "Extracting and building..." -ForegroundColor Blue
ssh -i "$KeyPath" ubuntu@$ElasticIP "cd /tmp && unzip -o clash-tracker-source.zip && ls -la"

Write-Host "Building Spring Boot JAR..." -ForegroundColor Blue
ssh -i "$KeyPath" ubuntu@$ElasticIP "cd /tmp && mvn clean package -DskipTests"
...
ssh -i "$KeyPath" ubuntu@$ElasticIP "cd /tmp/clash-war-tracker-frontend && npm install && npm run build"
...
Write-Host "Setting up application directories..." -ForegroundColor Blue
ssh -i "$KeyPath" ubuntu@$ElasticIP @"
sudo mkdir -p /opt/clash-tracker/{app,frontend,config,logs} &&
sudo cp /tmp/target/*.jar /opt/clash-tracker/app/clash-war-tracker-backend.jar &&
sudo cp -r /tmp/clash-war-tracker-frontend/dist/* /opt/clash-tracker/frontend/ &&
...
Write-Host "Creating application configuration..." -ForegroundColor Blue
ssh -i "$KeyPath" ubuntu@$ElasticIP @"
...
Write-Host "Creating systemd services..." -ForegroundColor Blue
ssh -i "$KeyPath" ubuntu@$ElasticIP @"
...
ssh -i "$KeyPath" ubuntu@$ElasticIP "sudo npm install -g serve"
